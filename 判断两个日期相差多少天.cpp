...
bool compare(people a , people b) //定义一个比较函数，比较姓名用于排序 
{
...
public:
	...
	void paixu() //按照姓名排序 
	{
		...
		if (num != 0)
		{ 
		   sort(student,student+num,compare); //利用sort函数进行排序 
		   for (int n = 0; n < num; n++)
		   {
			 cout << "*姓名：" << student[n].name << endl;
			 cout << "*生日：" << student[n].birthday[0] << "/" << student[n].birthday[1] << "/" << student[n].birthday[2] << endl;
